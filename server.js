...
app.use(cors({ origin: ['http://127.0.0.1:5500','http://localhost:5500','http://localhost:3000','https://mcorephysio.co.uk','https://www.mcorephysio.co.uk'] }));
app.use(express.json());
...
app.post('/sendmail', async (req,res) => {
  const { first_name, surname, age, sex, location_problem, email, preferred_time, service } = req.body;
  if(!first_name || !surname || !age || !sex || !location_problem || !email || !service) return res.status(400).json({message:'Please fill all required fields'});
...
app.post('/contact', async (req,res) => {
  const { username,email,message } = req.body;
  if(!username||!email||!message) return res.status(400).json({message:'Please fill all required fields'});
